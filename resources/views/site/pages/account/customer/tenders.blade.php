                    ...
                    @foreach($user->ownedTenders()->orderBy('created_at', 'desc')->get() as $tender)
                        <tr class="my-job-item">
                            <td>
                                ...
                                <div class="meta-job"><span> <i class="fas fa-calendar-alt"></i> {{ __('Опубликован:') }} @if ($tender->published) {{ \Carbon\Carbon::create($tender->published_at)->format('d.m.Y') }} @else {{ __('На модерации') }} @endif</span><span> <i
                                            class="fas fa-calendar-alt"></i>{{ __('Истекает') }} {{ $tender->deadline }}</span></div>
                                ...
                                <div class="job-info d-xl-none"><a href="{{ route('site.account.tenders.candidates', $tender->slug) }}"
                                        class="number-application">{{ $tender->requests()->count() }} {{ __('заявок') }}</a>  <span
                                        class="active">@if ($tender->isDeleted()) {{ __('Удален') }} @elseif($tender->opened) {{ __('Активный') }} @else {{ __('Закрыт') }} @endif</span></div>
                            ...
                            <td class="d-none d-xl-table-cell text-center active">@if ($tender->status !== 'done' && !$tender->isDeleted()) @if ($tender->checkDeadline()){{ __(' Открыт') }} @else @if($tender->owner_id && $tender->opened) {{ __('В разработке') }} @else {{ __('Приём заявок закрыт') }} @endif @endif @elseif ($tender->isDeleted()) {{ __('Удален') }} @else {{ __('Выполнен!') }} @endif</td>
